;  Call music_init once, then music_play exactly once per frame (PAL, 50 Hz)
;  from a raster IRQ.  The routine takes roughly 3-6 raster lines.
;  Entry points are the first two instructions so that the whole block can be
;  wrapped in a PSID header (init = base+0, play = base+3).

SID             = $d400
;  The MEGA65 has a SID per stereo channel, $d400 on the left and $d420 on the
;  right, so every write is made twice and the tune comes out of both. On a
;  C64 $d420 is a partly decoded mirror of $d400 and the second write lands
;  back on the same registers, which costs a few cycles and changes nothing.
SID2            = SID + $20

NUM_VOICES      = 3
SPEED           = 6             ; frames per pattern row (row = 1/16 note)
                                ; 50 / 6 = 8.33 rows per second = 125 BPM
...
; ---------------------------------------------------------------------------
;  music_init -- silence the SID, rewind all three voices
; ---------------------------------------------------------------------------
music_init:
                lda #0
                ldx #$18
