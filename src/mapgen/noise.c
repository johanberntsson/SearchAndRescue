...
//
// **It is written and it does not fit.** With colour_build called, `code`
// overflows by about 2.7 KB and no amount of moving BSS recovers that: stage
// one's text alone is 27.5 KB of the 32, its constants another 2.2, and the
// 12 KB under the banked-out ROMs can hold only BSS -- a PRG may not have two
// content areas. So this has never run and is unverified against `--stage
// colour`'s D69C51D9.
//
// **The answer is the one the design has had in reserve since the costing**:
// "if even that is not enough, the generator splits again -- terrain, then
// colour, then chain to the game". The handover mechanism for it already
// exists and is proved twice over; a second generator program would inherit
// the whole 32 KB, the tables, and the fields already sitting in attic RAM,
// and would need nothing new invented. See documentation/on-device-maps.md.
//
// **What the machine computes is the index, not the colour.** The RGB behind
// each entry is worked out once on the PC from maps/palette.yaml and ships as
// a 768-byte table; nothing here interpolates a colour stop or multiplies a
// channel by a shade. Only the per-pixel decision is ported, which is the half
// that has to run a quarter of a million times.
//
// **And the palette's layout is arithmetic, so there are no index tables.**
// The shared ramp puts 21 land entries at 24 and every sixth after it, eight
// water entries at 16, and two masonry entries at 150 and 156 -- six shades
// apiece. So `ramp[step] + face` is `24 + 6 * step + face` and the tables
// genmap.py builds are three multiply-adds here.
#define SHADES     6
#define RAMP_LEN   21
#define RAMP_BASE  24
#define DEEP_LEN   8
...
// The two dither lattices, at the map's own size over 16, and the finished
// indices. One byte a pixel, so a quarter of the field.
#define MOTTLE_PER  (SIZE / 16)
#define COLOUR_FIELD (MAP_SLOT(0) + 0x100000UL)
...
{
  uint8_t sh = lshift[OCT - 1] + 1;      // period 32 is half the finest octave's
  uint16_t stepmask = (uint16_t)((1 << sh) - 1);
  const uint16_t *w = wtab + woff[OCT - 1];
  uint16_t iy0 = y >> sh, ix0 = x >> sh;
...
uint32_t colour_build(void)
{
  ...
  win1 = work.edge[0][1];
  win2 = work.edge[0][2];
  bedrow = work.edge[0][3];
  lvrow = work.edge[1][0];
  btrow = work.edge[1][1];
  out = (uint8_t *)work.edge[1][2];
